...
class SingleOutcomeSubscriptionManager {
    ...
    private func handleSubscriptionContent(_ content: WAMPSubscriptionContent<EveryMatrix.AggregatorResponse>) throws -> SubscribableContent<Event>? {
        ...
        case .initialContent(let response):
            ...
            print("⚠️ [SingleOutcome] Failed to build event from initial content")
            return nil
        ...
        case .updatedContent(let response):
            ...
            print("⚠️ [SingleOutcome] Failed to build event from update")
            return nil
    ...
    private func buildSingleOutcomeEvent() -> Event? {
        print("⚙️ [SingleOutcome] Building event for eventId: \(eventId), outcomeId: \(outcomeId)")
        ...
        print("⚙️ [SingleOutcome] Store contents: \(matchCount) matches, \(marketCount) markets, \(outcomeCount) outcomes, \(bettingOfferCount) offers")
        ...
        print("⚙️ [SingleOutcome] Found match: \(matchDTO.name)")
        ...
        print("⚙️ [SingleOutcome] Built match with \(internalMatch.markets.count) markets")
        if let market = internalMatch.markets.first {
            print("⚙️ [SingleOutcome] Market has \(market.outcomes.count) outcomes")
            if let outcome = market.outcomes.first {
                print("⚙️ [SingleOutcome] Outcome has \(outcome.bettingOffers.count) betting offers")
    ...
    private func parseOutcomeData(from response: EveryMatrix.AggregatorResponse) {
        ...
        if !entityCounts.isEmpty {
            ...
            print("⚙️ [SingleOutcome] Parsed entities: \(summary)")
        }
    }
    ...
    private func handleOutcomeChangeRecord(_ change: EveryMatrix.ChangeRecord) {
        ...
        case .update:
            ...
            guard let changedProperties = change.changedProperties else {
                print("⚠️ [SingleOutcome] UPDATE change record missing changedProperties for \(change.entityType):\(change.id)")
                return
            }
