...
class GomaHomeContentAPIClient {
    ...
    /// Get footer links configured in CMS
    /// - Parameter language: Optional language override sent to the CMS (defaults to backend inference)
    func footerLinks(language: String?) -> AnyPublisher<GomaModels.FooterLinks, ServiceProviderError> {
        let endpoint = GomaHomeContentAPISchema.footerLinks(language: language)
        return self.connector.request(endpoint)
    }
