...
//                    print("TSManager getModel initial dump - proc:\(initialDumpProcedure) url:\(router.procedure), args:\(router.args ?? [] )")
//                }
//                else {
//                    print("TSManager getModel - url:\(router.procedure), args:\(router.args ?? [] ), kwargs:\(router.kwargs ?? [:] )")
//                }
...
//                        }
    ...
    func subscribeEndpoint<T: Decodable>(_ endpoint: TSRouter, decodingType: T.Type) -> AnyPublisher<TSSubscriptionContent<T>, EveryMatrix.APIError> {
        ...
        print("TSManager subscribeEndpoint - url:\(endpoint.procedure), args:\(args)")
    ...
    func registerOnEndpoint<T: Decodable>(_ endpoint: TSRouter, decodingType: T.Type) -> AnyPublisher<TSSubscriptionContent<T>, EveryMatrix.APIError> {
        ...
        print("TSManager registerOnEndpoint - url:\(endpoint.procedure), args:\(args)")
    ...
    func ssWampSessionConnected(_ session: SSWampSession, sessionId: Int) {
        ...
        sessionStateChanged()
            .sink(receiveCompletion: { completion in
                print("TSManager sessionStateChanged completion: \(completion)")
            }, receiveValue: { connected in
                print("TSManager sessionStateChanged \(connected)")
            })
