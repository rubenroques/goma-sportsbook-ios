...
public protocol LanguageSelectorViewModelProtocol {
    ...
    /// Callback closure invoked when a language is selected
    /// Used for flow communication (e.g., coordinator callbacks)
    var onLanguageSelected: ((LanguageModel) -> Void)? { get set }
