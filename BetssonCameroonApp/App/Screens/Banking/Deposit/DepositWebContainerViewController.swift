...
final class DepositWebContainerViewController: UIViewController {
    ...
    private func updateUI(for state: CashierFrameState) {
        ...
        case .loadingWebView(let url):
            ...
            print("💰 [Deposit] Cashier URL received: \(url.absoluteString)")
...
extension DepositWebContainerViewController: WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
        ...
        updateTimerOverlay()
        print("🌐 [Deposit] WebView started provisional navigation")
    }
    ...
    func webView(_ webView: WKWebView, didCommit navigation: WKNavigation!) {
        ...
        updateTimerOverlay()
        print("🌐 [Deposit] WebView committed navigation")
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        updateTimerOverlay()
        print("🌐 [Deposit] WebView finished loading DOM")
...
extension DepositWebContainerViewController {
    ...
    private func startSpinnerPolling() {
        ...
        print("🔍 [Deposit] Started polling for spinner removal")
        ...
        spinnerPollingTimer = Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true) { [weak self] timer in
            ...
            self.webView.evaluateJavaScript(javascript) { result, error in
                if let isRemoved = result as? Bool, isRemoved {
                    ...
                    print("✅ [Deposit] Spinner removed - webpage fully loaded (poll count: \(pollCount))")
                    self.onSpinnerRemoved()
                } else if pollCount >= 60 {
                    // Timeout after 30 seconds (60 polls * 0.5s)
                    print("⏱️ [Deposit] Spinner polling timeout after 30 seconds")
    ...
    private func onSpinnerRemoved() {
        ...
        if let metrics = timingMetrics {
            print("📊 [Deposit] Final Timing Breakdown:")
    ...
    private func onSpinnerPollingTimeout() {
        ...
        print("⚠️ [Deposit] Spinner polling timed out, marking as ready anyway")
    }
}
