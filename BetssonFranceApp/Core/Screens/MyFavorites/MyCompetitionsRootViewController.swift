//
//  MyCompetitionsRootViewController.swift
//  Sportsbook
//
//  Created by André Lascas on 24/08/2023.
//
...
class MyCompetitionsRootViewController: UIViewController {
    ...
    private var viewModel: MyCompetitionsRootViewModel
    ...
    init(viewModel: MyCompetitionsRootViewModel) {
        self.viewModel = MyCompetitionsRootViewModel()
    ...
    override func viewDidLoad() {
            ...
            .sink { [weak self] filterApplied in
                ...
                if let viewControllers = self?.viewControllers {
                    if viewControllers.isEmpty {
                        ...
                        let allMyCompetitionsViewModel = MyCompetitionsViewModel(myCompetitionsTypeList: .all)
                        let allMyCompetitionsViewController = MyCompetitionsViewController(viewModel: allMyCompetitionsViewModel)

                        let liveMyCompetitionsViewModel = MyCompetitionsViewModel(myCompetitionsTypeList: .live)
                        let liveMyCompetitionsViewController = MyCompetitionsViewController(viewModel: liveMyCompetitionsViewModel)

                        let todayMyCompetitionsViewModel = MyCompetitionsViewModel(myCompetitionsTypeList: .today)
                        let todayMyCompetitionsViewController = MyCompetitionsViewController(viewModel: todayMyCompetitionsViewModel)

                        let tomorrowMyCompetitionsViewModel = MyCompetitionsViewModel(myCompetitionsTypeList: .tomorrow)
                        let tomorrowMyCompetitionsViewController = MyCompetitionsViewController(viewModel: tomorrowMyCompetitionsViewModel)

                        let thisWeekMyCompetitionsViewModel = MyCompetitionsViewModel(myCompetitionsTypeList: .thisWeek)
                        let thisWeekMyCompetitionsViewController = MyCompetitionsViewController(viewModel: thisWeekMyCompetitionsViewModel)

                        let nextWeekMyCompetitionsViewModel = MyCompetitionsViewModel(myCompetitionsTypeList: .nextWeek)
                        let nextWeekMyCompetitionsViewController = MyCompetitionsViewController(viewModel: nextWeekMyCompetitionsViewModel)

                        self?.viewControllers = [
                            allMyCompetitionsViewController,
                            liveMyCompetitionsViewController,
                            todayMyCompetitionsViewController,
                            tomorrowMyCompetitionsViewController,
                            thisWeekMyCompetitionsViewController,
                            nextWeekMyCompetitionsViewController
                        ]
                    }
                    else {
                        for viewController in viewControllers {
                            let myCompetitionsViewController = viewController as? MyCompetitionsViewController

                            myCompetitionsViewController?.reloadDataWithFilter(newFilter: filterApplied)
                        }
                    }
                }
        ...
        self.viewModel.sendLoadingStatus = { [weak self] in
            ...
            for viewController in self.viewControllers {
                ...
                myCompetitionsViewController?.viewModel.isLoadingPublisher.send(true)

            }
        }

        self.viewModel.shouldUpdateContent = { [weak self] in

            guard let self = self else { return }
            for viewController in self.viewControllers {
                let myCompetitionsViewController = viewController as? MyCompetitionsViewController

                let competitions = self.viewModel.favoriteCompetitionsDataPublisher.value
                let outrightCompetitions = self.viewModel.favoriteOutrightCompetitionsDataPublisher.value

                switch myCompetitionsViewController?.viewModel.myCompetitionsTypeList {
                case .all:
                    myCompetitionsViewController?.viewModel.updateContentData(competitions: competitions, outrightCompetitions: outrightCompetitions)
                case .live:
                    var filteredCompetitions = competitions

                    var filteredOutrightCompetitions = outrightCompetitions

                    for index in filteredCompetitions.indices {
                        filteredCompetitions[index].matches = filteredCompetitions[index].matches.filter {
                            self.viewModel.isDateLive($0.date ?? Date())
                        }
                    }

                    filteredCompetitions = filteredCompetitions.filter {
                        $0.matches.isNotEmpty
                    }

                    for index in filteredOutrightCompetitions.indices {
                        filteredOutrightCompetitions[index].matches = filteredOutrightCompetitions[index].matches.filter {
                            self.viewModel.isDateLive($0.date ?? Date())
                        }
                    }

                    filteredOutrightCompetitions = filteredOutrightCompetitions.filter {
                        $0.matches.isNotEmpty
                    }

                    myCompetitionsViewController?.viewModel.updateContentData(competitions: filteredCompetitions, outrightCompetitions: filteredOutrightCompetitions)
                ...
                case .today:
                    ...
                    for index in filteredCompetitions.indices {
                        filteredCompetitions[index].matches = filteredCompetitions[index].matches.filter {
                            self.viewModel.isDateToday($0.date ?? Date())
                        }
                    }
                ...
                case .tomorrow:
                    ...
                    for index in filteredCompetitions.indices {
                        filteredCompetitions[index].matches = filteredCompetitions[index].matches.filter {
                            self.viewModel.isDateTomorrow($0.date ?? Date())
                        }
                    }
                ...
                case .thisWeek:
                    ...
                    for index in filteredCompetitions.indices {
                        filteredCompetitions[index].matches = filteredCompetitions[index].matches.filter {
                            self.viewModel.isDateInThisWeek($0.date ?? Date())
                        }
                    }
                ...
                case .nextWeek:
                    ...
                    for index in filteredCompetitions.indices {
                        filteredCompetitions[index].matches = filteredCompetitions[index].matches.filter {
                            self.viewModel.isDateInNextWeek($0.date ?? Date())
                        }
                    }
        ...
        Env.betslipManager.bettingTicketsPublisher
            ...
            .sink(receiveValue: { [weak self] _ in
                ...
                for viewController in self.viewControllers {
                    if let viewController = viewController as? MyCompetitionsViewController {

                        if let currentVCIndex = self.viewModel.selectedIndexPublisher.value {

                            if currentVCIndex != viewController.viewModel.myCompetitionsTypeList.index {
        ...
        self.viewModel.setupPublishers()
    ...
    // MARK: - Bindings
    private func bind(toViewModel viewModel: MyCompetitionsRootViewModel) {
...
extension MyCompetitionsRootViewController: UIPageViewControllerDelegate, UIPageViewControllerDataSource {
...
extension MyCompetitionsRootViewController: UICollectionViewDelegate, UICollectionViewDataSource {
...
extension MyCompetitionsRootViewController {
