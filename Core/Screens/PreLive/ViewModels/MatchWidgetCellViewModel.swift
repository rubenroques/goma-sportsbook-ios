...
// Global timing reference for debugging
class TimingDebug {
    static let referenceDate = Date()

    static func millisSinceReference() -> Int {
        return Int(Date().timeIntervalSince(referenceDate) * 1000)
    }

    static func log(_ message: String) {
        print("DELAYBUG: [\(millisSinceReference()) ms] \(message)")
    }
}
...
class MatchWidgetCellViewModel {
    ...
    private let oldBoostedOddOutcomeSubject: CurrentValueSubject<BoostedOutcome?, Never> = .init(nil)
    var oldBoostedOddOutcomePublisher: AnyPublisher<BoostedOutcome?, Never> {
        return oldBoostedOddOutcomeSubject.eraseToAnyPublisher()
    }

    var oldBoostedOddOutcome: BoostedOutcome? {
        return oldBoostedOddOutcomeSubject.value
    }

    private let matchWidgetStatusSubject: CurrentValueSubject<MatchWidgetStatus, Never> = .init(.unknown)
    var matchWidgetStatusPublisher: AnyPublisher<MatchWidgetStatus, Never> {
        return matchWidgetStatusSubject.eraseToAnyPublisher()
    }

    var matchWidgetStatus: MatchWidgetStatus {
        return matchWidgetStatusSubject.value
    }

    private let matchWidgetTypeSubject: CurrentValueSubject<MatchWidgetType, Never> = .init(.normal)
    ...
    var matchWidgetType: MatchWidgetType {
        return matchWidgetTypeSubject.value
    }
    ...
    init(match: Match, matchWidgetType: MatchWidgetType = .normal, matchWidgetStatus: MatchWidgetStatus = .unknown) {
        ...
        Publishers.CombineLatest(self.matchMarketsSubject, self.matchLiveDataSubject)
            ...
            .sink { [weak self] updatedMatch in
                self?.matchSubject.send(updatedMatch)
            }
    ...
    private func loadBoostedOddOldValueIfNeeded() {
        ...
        .sink { _ in
               ...
               let outcome = market.outcomes.first(where: { outcome in outcome.typeName.lowercased() == firstCurrentOutcomeName })
            {
                ...
                self?.oldBoostedOddOutcomeSubject.send(BoostedOutcome(type: "home", name: firstCurrentOutcomeName, valueAttributedString: attributedString))
            }
               ...
               let outcome = market.outcomes.first(where: { outcome in outcome.typeName.lowercased() == secondCurrentOutcomeName })
            {
                ...
                let attributes = [NSAttributedString.Key.strikethroughStyle: NSUnderlineStyle.single.rawValue]
                let attributedString = NSAttributedString(string: oddValue, attributes: attributes)
                self?.oldBoostedOddOutcomeSubject.send(BoostedOutcome(type: "draw", name: secondCurrentOutcomeName, valueAttributedString: attributedString))
            }
               ...
               let outcome = market.outcomes.first(where: { outcome in outcome.typeName.lowercased() == thirdCurrentOutcomeName })
            {
                ...
                let attributes = [NSAttributedString.Key.strikethroughStyle: NSUnderlineStyle.single.rawValue]
                let attributedString = NSAttributedString(string: oddValue, attributes: attributes)
                self?.oldBoostedOddOutcomeSubject.send(BoostedOutcome(type: "away", name: thirdCurrentOutcomeName, valueAttributedString: attributedString))
            }
            else {
                self?.oldBoostedOddOutcomeSubject.send(BoostedOutcome())
            }
        }
        .store(in: &self.cancellables)
    }
