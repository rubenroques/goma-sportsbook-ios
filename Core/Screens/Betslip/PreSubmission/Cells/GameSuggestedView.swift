...
class GameSuggestedView: NibView {
    ...
    func setupWithTheme() {
        ...
        self.gameInfoLabel.textColor = UIColor.App2.textPrimary
       // self.gameInfoLabel.font = AppFont.with(type: .medium, size: 11)

        self.gameTitleLabel.textColor = UIColor.App2.textPrimary
        // self.gameTitleLabel.font = AppFont.with(type: .bold, size: 13)
        if let gameView = self.gameView {
            gameView.backgroundColor = UIColor.App2.backgroundCards
        }
