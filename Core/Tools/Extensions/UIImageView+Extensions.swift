...
extension UIImageView {
    ...
    func autofillBackgroundColor() {
        ...
        guard
            let imageToCrop = self.image,
            let cgImage = imageToCrop.cgImage
        else { return }
        
        let size = imageToCrop.size
        
        guard
            let cornerImageRef = cgImage.cropping(to: CGRect(x: 0, y: 0, width: 2, height: 2))
        else { return }
        let cornerCropped: UIImage = UIImage(cgImage: cornerImageRef)
        
        if let averageCornerColor = cornerCropped.averageColor, averageCornerColor.cgColor.alpha == 1.0 {
            self.backgroundColor = averageCornerColor
            return
        }
        
        guard
            let middleImageRef = cgImage.cropping(to: CGRect(x: size.width/2, y: 0, width: 2, height: 2))
        else { return }
        let middleCropped: UIImage = UIImage(cgImage: middleImageRef)
        
        if let averageMiddleColor = middleCropped.averageColor, averageMiddleColor.cgColor.alpha == 1.0 {
            self.backgroundColor = averageMiddleColor
        }
