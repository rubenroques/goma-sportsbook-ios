//
//  SuggestedBetCardCollectionViewCell.swift
//  Sportsbook
//
...
class SuggestedBetCardCollectionViewCell: UICollectionViewCell {
    ...
    var viewModel: SuggestedBetCardViewModel?

    var openSuggestedBetDetailAction: ((SuggestedBetCardViewModel) -> Void) = { _ in }
    ...
    func configure(viewModel: SuggestedBetCardViewModel, socialFeaturesEnabled: Bool = false, hasCounter: Bool, followingUsers: [Follower]) {
        ...
        for (i, featuredTipSelection) in viewModel.selectionViewModels.enumerated() {
            if i >= SuggestedBetCarouselViewModel.maxTicketsBeforeExpand && (self.viewModel?.shouldCropList ?? true) {
                self.showFullTipButton = true
                break
            }
            else {
                let tipView = SuggestedBetSelectionView()
                tipView.configure(withSuggestedBetSelectionViewModel: featuredTipSelection)
    ...
    @objc func didTapShowFullTipButton() {
        ...
        if let viewModel = self.viewModel {
            self.openSuggestedBetDetailAction(viewModel)
        }
    }
...
extension SuggestedBetCardCollectionViewCell {
