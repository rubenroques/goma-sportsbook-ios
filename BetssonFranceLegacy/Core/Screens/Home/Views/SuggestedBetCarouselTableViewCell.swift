//
//  SuggestedBetCarouselTableViewCell.swift
//  Sportsbook
//
...
class SuggestedBetCarouselViewModel {
    ...
    var suggestedBetCardCacheViewModel: [String: SuggestedBetCardViewModel] = [:]
...
class SuggestedBetCarouselTableViewCell: UITableViewCell {
    ...
    private var viewModel: SuggestedBetCarouselViewModel?
    private var cancellables: Set<AnyCancellable> = []

    var openSuggestedBetDetailAction: ((SuggestedBetCardViewModel) -> Void)?
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.collectionView.register(SuggestedBetCardCollectionViewCell.self,
                                        forCellWithReuseIdentifier: SuggestedBetCardCollectionViewCell.identifier)
    ...
    func configure(withViewModel viewModel: SuggestedBetCarouselViewModel) {

        self.viewModel = viewModel

        if viewModel.maxTicketsCount() > SuggestedBetCarouselViewModel.maxTicketsBeforeExpand {
...
extension SuggestedBetCarouselTableViewCell: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard
            let cell = collectionView.dequeueCellType(SuggestedBetCardCollectionViewCell.self, indexPath: indexPath),
            let cellViewModel = self.viewModel?.cellViewModel(forIndex: indexPath.row)
        else {
            fatalError()
        }

        cell.configure(viewModel: cellViewModel, hasCounter: false, followingUsers: [])

        cell.openSuggestedBetDetailAction = { [weak self] featuredTip in
            self?.openSuggestedBetDetailAction?(featuredTip)
        }
...
extension SuggestedBetCarouselTableViewCell: UIScrollViewDelegate {
...
extension SuggestedBetCarouselTableViewCell {
