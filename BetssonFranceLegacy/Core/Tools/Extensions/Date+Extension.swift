...
//  All Goals
//
//  Created by Ruben Roques on 27/09/2019.
...
extension Date {
     ...
     */
    //    init?(fromString string: String, format:DateFormatType, timeZone: TimeZoneType = .local, locale: Locale = Env.locale, isLenient: Bool = true) {
    ...
    /// Converts the date to string based on DateFormatter's date style and time style
    /// with optional relative date formatting, optional time zone and optional locale.
    func toString(dateStyle: DateFormatter.Style, timeStyle: DateFormatter.Style,
                  isRelative: Bool = false, timeZone: Foundation.TimeZone = Foundation.NSTimeZone.local,
                  locale: Locale = Env.locale) -> String {

        let formatter = Date.cachedDateFormatters.cachedFormatter(dateStyle, timeStyle: timeStyle,
    ...
    func toStringWithRelativeTime(strings: [RelativeTimeStringType: String]? = nil) -> String {
        ...
        let now = Env.date().timeIntervalSince1970
    ...
    func compare(_ comparison: DateComparisonType) -> Bool {
        ...
        case .isTomorrow:
            let comparison = today.adjust(.day, offset: 1)
        ...
        case .isNextWeek:
            let comparison = today.adjust(.week, offset: 1)
        ...
        case .isNextMonth:
            let comparison = today.adjust(.month, offset: 1)
        ...
        case .isThisYear:
            return self.compare(.isSameYear(as: today))
        case .isNextYear:
            let comparison = today.adjust(.year, offset: 1)
        ...
        case .isWeekend:
            let range = Env.calendar.maximumRange(of: Calendar.Component.weekday)!
    ...
    func adjust(_ component: DateComponentType, offset: Int) -> Date {
        ...
        return Env.calendar.date(byAdding: dateComp, to: self)!
    }
    ...
    func adjust(hour: Int? = nil, minute: Int? = nil, second: Int? = nil, day: Int? = nil, month: Int? = nil) -> Date {
        ...
        return Env.calendar.date(from: comp)!
    }

    /// Return a new Date object with the new hour, minute and seconds values.
    func moveBy(hour: Int? = nil, minute: Int? = nil, second: Int? = nil, day: Int? = nil, month: Int? = nil) -> Date {
        var comp = Date.components(self)

        if let monthValue = month, let originalMonth = comp.month {
            comp.month = originalMonth + monthValue
        }
        if let dayValue = day, let originalDay = comp.day {
            comp.day = originalDay + dayValue
        }
        if let hourValue = hour, let originalHour = comp.hour {
            comp.hour = originalHour + hourValue
        }
        if let minuteValue = minute, let originalMinute = comp.minute {
            comp.minute = originalMinute + minuteValue
        }
        if let secondValue = second, let originalSecond = comp.second {
            comp.second = originalSecond + secondValue
        }
        return Env.calendar.date(from: comp)!
    }

    // MARK: Date for...
    func dateFor(_ type: DateForType, calendar: Calendar) -> Date {
        switch type {
        case .startOfDay:
            return adjust(hour: 0, minute: 0, second: 0)
        case .midDay:
            return adjust(hour: 12, minute: 0, second: 0)
        case .endOfDay:
    ...
    func numberOfDaysInMonth() -> Int {
        let range = Env.calendar.range(of: Calendar.Component.day, in: Calendar.Component.month, for: self)!
    ...
    internal static func components(_ fromDate: Date) -> DateComponents {
        return Env.calendar.dateComponents(Date.componentFlags(), from: fromDate)
    }
    ...
    internal class ConcurrentFormatterCache {
        ...
        func cachedFormatter(_ format: String = DateFormatType.standard.stringFormat,
                             ...
                             locale: Locale = Env.locale, isLenient: Bool = true) -> DateFormatter {
        ...
        func cachedFormatter(_ dateStyle: DateFormatter.Style,
                             timeStyle: DateFormatter.Style,
                             doesRelativeDateFormatting: Bool,
                             timeZone: Foundation.TimeZone = Env.timezone,
                             locale: Locale = Env.locale,
                             isLenient: Bool = true) -> DateFormatter {
...
enum TimeZoneType {
    ...
    var timeZone: TimeZone {
        ...
        case let .custom(gmt):
            return TimeZone(secondsFromGMT: gmt)!
        }
    }
}
...
enum DateForType {
    case startOfDay, midDay, endOfDay, startOfWeek, endOfWeek, startOfMonth, endOfMonth, tomorrow, yesterday, nearestMinute(minute: Int), nearestHour(hour: Int)
}
