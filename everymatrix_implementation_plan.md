...
This document outlines the tasks required to implement a new service provider connector for **EveryMatrix** within the `ServicesProvider` framework. The goal is to create a fully functional connector supporting WebSocket communication, conforming to all framework protocols (`Connector`, `EventsProvider`, `BettingProvider`, `PrivilegedAccessManager`).

**Key Update:** The EveryMatrix API will be WebSocket-only with no REST API endpoints. All functionality must be implemented through WebSocket channels.

**Key Technologies:** Swift, Combine, Protocol-Oriented Programming, WebSockets

**Reference Implementation:** Refer to the existing `SportRadar` provider (`ServicesProvider/Sources/ServicesProvider/Providers/Sportsradar/`) for examples.

**Story Point Scale:** 1 (trivial), 2 (simple), 3 (moderate), 5 (complex), 8 (very complex), 13 (large), 21 (very large)

---

## Phase 1: Foundation & Configuration

### Task 1.1: Setup Project Structure [1]
*   **Goal:** Create the standard directory layout for the new EveryMatrix provider.
*   **Subtasks:**
...
*   **Acceptance:** Directory structure matches standard provider layout.

### Task 1.2: Implement Basic Configuration [3]
*   **Goal:** Integrate EveryMatrix into the framework's configuration system.
*   **Subtasks:**
...
*   **Acceptance:** Running `ServicesProviderClient(providerType: .everyMatrix, configuration: config)` successfully initializes an EveryMatrix client with correct settings.

### Task 1.3: Implement Base Connector [2]
*   **Goal:** Create the fundamental `EveryMatrixConnector` class.
*   **Subtasks:**
...
*   **Acceptance:** `EveryMatrixConnector` successfully publishes connection state changes through its `connectionStatePublisher`.

### Task 1.4: Foundation Testing [2]
*   **Goal:** Verify the configuration and base connector implementation.
*   **Subtasks:**
...
*   **Acceptance:** Configuration and base connector components pass tests with >90% code coverage.

---

## Phase 2: WebSocket Core Infrastructure

### Task 2.1: WebSocket Client Protocol [3]
*   **Goal:** Define and implement the core WebSocket client protocol that will interface with EveryMatrix WebSocket API.
*   **Subtasks:**
...
*   **Acceptance:** Protocol contains a complete interface for WebSocket operations without implementation gaps.

### Task 2.2: WebSocket Connection Implementation [8]
*   **Goal:** Implement a concrete WebSocket client that connects to the EveryMatrix WebSocket API.
*   **Subtasks:**
...
*   **Acceptance:** Client successfully connects to EveryMatrix WebSocket endpoint, authenticates, maintains connection, and handles reconnection scenarios.

### Task 2.3: Request-Response Pattern for WebSocket [5]
*   **Goal:** Implement a request-response pattern over WebSocket to replace the functionality typically handled by REST.
*   **Subtasks:**
...
*   **Acceptance:** Request-response pattern allows for making API calls over WebSocket with proper response handling.

### Task 2.4: DTOs & Normalized Data Models [5]
*   **Goal:** Define data transfer objects that match the flat/normalized data structure sent by EveryMatrix WebSocket.
*   **Subtasks:**
...
*   **Acceptance:** Sample messages from EveryMatrix WebSocket documentation successfully decode into the DTO structures without parsing errors.

### Task 2.5: Channel Data Store Implementation [13]
*   **Goal:** Create the core data storage and reconstruction component for a single WebSocket channel.
*   **Subtasks:**
...
*   **Acceptance:** `ChannelDataStore` successfully processes sample WebSocket messages, maintains correct relationship information, and emits properly reconstructed object hierarchies.

### Task 2.6: WebSocket Provider Integration [8]
*   **Goal:** Connect WebSocket components to the EveryMatrix provider implementation.
*   **Subtasks:**
...
*   **Acceptance:** Provider base class correctly manages WebSocket connections, channels, and request-response operations.

### Task 2.7: Performance Optimization & Memory Management [8]
*   **Goal:** Optimize the WebSocket implementation for high-performance and efficient memory usage.
*   **Subtasks:**
...
*   **Acceptance:** Implementation successfully processes 100+ updates per second without excessive CPU usage or memory growth.

### Task 2.8: WebSocket Infrastructure Testing [5]
*   **Goal:** Create comprehensive tests for the WebSocket infrastructure components.
*   **Subtasks:**
...
*   **Acceptance:** All WebSocket infrastructure components pass tests with >90% code coverage.

---

## Phase 3: Events Provider Implementation

### Task 3.1: Define Events Provider Structure & Domain Models [5]
*   **Goal:** Set up the `EveryMatrixEventsProvider` class and define the domain models for events data.
*   **Subtasks:**
...
*   **Acceptance:** Provider class compiles and domain models properly represent the public API expected by consumers.

### Task 3.2: Implement WebSocket Channel Definitions for Events [5]
*   **Goal:** Define and implement WebSocket channels for different types of events data.
*   **Subtasks:**
...
*   **Acceptance:** All event data subscription types map to appropriate WebSocket channels with correct parameters.

### Task 3.3: Implement Subscription Methods [8]
*   **Goal:** Implement the subscription methods using the channel data store architecture.
*   **Subtasks:**
...
*   **Acceptance:** All implemented subscription methods properly subscribe to WebSocket channels and return typed publishers that emit updates.

### Task 3.4: Implement WebSocket Request Methods for Events [8]
*   **Goal:** Implement non-subscription methods that would typically use REST, using the WebSocket request-response pattern.
*   **Subtasks:**
...
*   **Acceptance:** All non-subscription `EventsProvider` methods successfully operate via WebSocket requests and return expected data.

### Task 3.5: Domain Model Mapping & Event Reconstruction [5]
*   **Goal:** Implement the mapping logic from DTO objects to domain models.
*   **Subtasks:**
...
*   **Acceptance:** Mapping functions successfully convert complete and partial DTO object graphs to valid domain models.

### Task 3.6: EventsProvider Testing [5]
*   **Goal:** Verify the implementation of the `EveryMatrixEventsProvider`.
*   **Subtasks:**
...
*   **Acceptance:** Tests provide >80% code coverage of the `EveryMatrixEventsProvider` implementation.

---

## Phase 4: Betting Provider Implementation

### Task 4.1: Define Betting Provider Structure & Models [5]
*   **Goal:** Set up the `EveryMatrixBettingProvider` class and define necessary betting data models.
*   **Subtasks:**
...
*   **Acceptance:** Provider structure is established with appropriate model definitions that match EveryMatrix API formats.

### Task 4.2: Implement Betting Request-Response Methods [8]
*   **Goal:** Implement WebSocket request methods for betting operations.
*   **Subtasks:**
...
*   **Acceptance:** Betting operations successfully execute via WebSocket and return appropriate responses.

### Task 4.3: Implement Betting WebSocket Subscriptions [8]
*   **Goal:** Implement real-time betting functionality using WebSocket subscriptions.
*   **Subtasks:**
...
*   **Acceptance:** Real-time betting features receive and process updates through WebSocket subscriptions.

### Task 4.4: Implement Betting Mappers [3]
*   **Goal:** Create mapping logic to convert EveryMatrix betting models to canonical models.
*   **Subtasks:**
...
*   **Acceptance:** Mapping functions convert all EveryMatrix betting DTOs to canonical framework models.

### Task 4.5: Implement Remaining BettingProvider Methods [5]
*   **Goal:** Complete the implementation of the `BettingProvider` protocol.
*   **Subtasks:**
...
*   **Acceptance:** All `BettingProvider` methods are implemented and return data in the expected format.

### Task 4.6: BettingProvider Testing [5]
*   **Goal:** Verify the implementation of the `EveryMatrixBettingProvider`.
*   **Subtasks:**
...
*   **Acceptance:** Tests provide >80% coverage of the `EveryMatrixBettingProvider` implementation.

---

## Phase 5: Privileged Access Manager (PAM) Implementation

### Task 5.1: Define PAM Structure & Core Models [5]
*   **Goal:** Set up `EveryMatrixPrivilegedAccessManager` and define core PAM-related models.
*   **Subtasks:**
...
*   **Acceptance:** Provider structure is established with necessary properties and initial model definitions.

### Task 5.2: Implement Authentication & User Profile via WebSocket [8]
*   **Goal:** Implement authentication and profile management using WebSocket requests.
*   **Subtasks:**
...
*   **Acceptance:** Authentication and user profile management successfully operate via WebSocket.

### Task 5.3: Implement PAM WebSocket Subscriptions [5]
*   **Goal:** Implement real-time user account features using WebSocket subscriptions.
*   **Subtasks:**
...
*   **Acceptance:** Real-time user account features receive and process updates through WebSocket.

### Task 5.4: Implement PAM Mappers [3]
*   **Goal:** Create mapping logic for PAM models.
*   **Subtasks:**
...
*   **Acceptance:** Mapping functions convert EveryMatrix PAM DTOs to canonical framework models.

### Task 5.5: Implement PAM Payments Functionality via WebSocket [8]
*   **Goal:** Implement payment functionality using WebSocket requests.
*   **Subtasks:**
...
*   **Acceptance:** All payment features function correctly using WebSocket requests and subscriptions.

### Task 5.6: Implement Remaining PAM Functionality [13]
*   **Goal:** Implement all other required PAM features via WebSocket.
*   **Subtasks:**
...
*   **Acceptance:** All PAM features from the protocol specification function correctly via WebSocket.

### Task 5.7: PAM Testing [8]
*   **Goal:** Verify the implementation of the `EveryMatrixPrivilegedAccessManager`.
*   **Subtasks:**
    *   [ ] Write unit tests for mapping functions.
...
*   **Acceptance:** Tests provide >80% coverage of the `EveryMatrixPrivilegedAccessManager` implementation.

---

## Phase 6: Integration & Finalization

### Task 6.1: Client Integration [5]
*   **Goal:** Make all EveryMatrix providers work together through the `ServicesProviderClient`.
*   **Subtasks:**
...
*   **Acceptance:** `ServicesProviderClient` correctly initializes and operates all EveryMatrix providers using a unified WebSocket connection.

### Task 6.2: WebSocket Architecture Optimization [8]
*   **Goal:** Fine-tune the WebSocket implementation based on testing results.
*   **Subtasks:**
...
*   **Acceptance:** WebSocket implementation handles realistic workloads with acceptable CPU (<10% on test device) and memory (no continuous growth) usage.

### Task 6.3: Error Handling Standardization [3]
*   **Goal:** Create a consistent error handling system across the provider.
*   **Subtasks:**
...
*   **Acceptance:** All errors from WebSocket and internal logic map to appropriate `ServiceProviderError` cases with descriptive messages.

### Task 6.4: Modular Provider Support [3]
*   **Goal:** Allow the framework to function when only some providers are initialized.
*   **Subtasks:**
...
*   **Acceptance:** Framework operates correctly with any combination of enabled/disabled providers.

### Task 6.5: Integration Testing [5]
*   **Goal:** Verify that all components work together correctly in realistic scenarios.
*   **Subtasks:**
...
*   **Acceptance:** Integration tests pass consistently with acceptable performance metrics.

### Task 6.6: Documentation [5]
*   **Goal:** Provide comprehensive documentation for the EveryMatrix implementation.
*   **Subtasks:**
...
*   **Acceptance:** Documentation provides clear guidance for usage and maintenance of the implementation.

---

**End Note:** This plan has been revised to focus exclusively on WebSocket implementation since the EveryMatrix API will not include REST endpoints. All functionality must be implemented through WebSocket connections, using both subscription-based patterns for real-time data and request-response patterns for operations traditionally handled by REST. The total story point estimate for the project is approximately **175**, suggesting 17-22 weeks of development time for a junior developer.